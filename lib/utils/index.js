...
// 可以改成自定义宏替换
const tplMapReplace = (options, str) => {
    const map = {
        PROJECTNAME: options.projectName,
    };
    if (options.CNprojectName) {
        map.CNPROJECTNAME = options.CNprojectName;
    }
    const reStr = '(' + Object.keys(map).map(it => '\\' + it).join('|') + ')';
    return str.replace(new RegExp(reStr, 'g'), it => map[it]);
}

const copyFile = function (srcPath, tarPath, cb, replaceRule) {
    // 根据path替换文件名
    const { projectName, CNprojectName } = replaceRule;
    // 异步读取
    fs.readFile(srcPath, 'utf-8', (error, data) => {
        if (!error) {
            let str = tplMapReplace({ projectName, CNprojectName }, data.toString()); // 宏替换
